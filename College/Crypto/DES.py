...
class DesKey:
    def __init__(self, key):
        self.key = key
        self.keys = list(derive_keys(key))

    def is_single(self):
        return len(self.key) == 8

    def is_triple(self):
        return len(self.key) in (16, 24)
    ...
    def encrypt(self, message, initial=None, padding=False):
        ...
        if initial is not None:
            if isinstance(initial, int):
                initial = struct.pack(">Q", initial)
            message = self._cbc_encrypt(message, initial)
        else:
            message = self._ecb_encrypt(message)
        return message
    ...
    def decrypt(self, message, initial=None, padding=False):
        if initial is not None:
            if isinstance(initial, int):
                initial = struct.pack(">Q", initial)
            message = self._cbc_decrypt(message, initial)
        else:
            message = self._ecb_decrypt(message)
        if padding:
    ...
    def _ecb_encrypt(self, message):
        return b"".join(
            struct.pack(
                ">Q", encode_block(struct.unpack(">Q", block)[0], self.keys, True)
            )
            for block in self._chunks(message, 8)
        )

    def _ecb_decrypt(self, message):
        return b"".join(
            struct.pack(
                ">Q", encode_block(struct.unpack(">Q", block)[0], self.keys, False)
            )
            for block in self._chunks(message, 8)
        )

    def _cbc_encrypt(self, message, iv):
        prev = struct.unpack(">Q", iv)[0]
        ciphertext = []
        for block in self._chunks(message, 8):
            block = struct.unpack(">Q", block)[0] ^ prev
            block = encode_block(block, self.keys, True)
            prev = block
            ciphertext.append(struct.pack(">Q", block))
        return b"".join(ciphertext)

    def _cbc_decrypt(self, message, iv):
        prev = struct.unpack(">Q", iv)[0]
        plaintext = []
        for block in self._chunks(message, 8):
            (block,) = struct.unpack(">Q", block)
            plaintext.append(
                struct.pack(">Q", encode_block(block, self.keys, False) ^ prev)
            )
            prev = block
...
if __name__ == "__main__":
    key = DesKey(b"Hi there")
    encrypted = key.encrypt(b"Dev Sanghvi", padding=True)
